...
export const ideasRouter = createTRPCRouter({
  ...
  list: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (input?.tags?.length) {
        ideas = ideas.filter((i) => input.tags!.some((tag) => i.tags.includes(tag)));
      }
  ...
  getWithRelations: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const authors = i.authors.map((id) => ctx.graph.getNode(id)).filter(Boolean);
      const periods = i.periods.map((id) => ctx.graph.getNode(id)).filter(Boolean);
      const influences = i.influences.map((id) => ctx.graph.getNode(id)).filter(Boolean);
      const influenced = i.influenced.map((id) => ctx.graph.getNode(id)).filter(Boolean);
      const contradicts = i.contradicts.map((id) => ctx.graph.getNode(id)).filter(Boolean);
      const synthesizes = i.synthesizes.map((id) => ctx.graph.getNode(id)).filter(Boolean);
  ...
  getInfluences: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return idea.influences.map((id) => ctx.graph.getNode(id)).filter(Boolean);
    }),
  ...
  getInfluenced: publicProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return idea.influenced.map((id) => ctx.graph.getNode(id)).filter(Boolean);
    }),
